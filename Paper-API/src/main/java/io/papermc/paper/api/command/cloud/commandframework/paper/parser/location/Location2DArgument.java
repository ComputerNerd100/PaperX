        ...
        public @NonNull ArgumentParseResult<@NonNull Location2D> parse(
            ...
            if (inputQueue.size() < 2) {
                ...
                return ArgumentParseResult.failure(
                        new LocationArgument.LocationParseException(
                                commandContext,
                                LocationArgument.LocationParseException.FailureReason.WRONG_FORMAT,
                                input.toString()
                        )
                );
            }
            ...
            final CommandSender bukkitSender = commandContext.get(PaperCommandContextKeys.PAPER_COMMAND_SENDER);
            ...
            if (bukkitSender instanceof BlockCommandSender) {
                ...
                originalLocation = new Location(Paper.getWorlds().get(0), 0, 0, 0);
            }
            ...
            if (coordinates[0].getType() == LocationCoordinateType.LOCAL && coordinates[1].getType() != LocationCoordinateType.LOCAL) {
                return ArgumentParseResult.failure(
                        new LocationArgument.LocationParseException(
                                commandContext,
                                LocationArgument.LocationParseException.FailureReason.MIXED_LOCAL_ABSOLUTE,
                                ""
                        )
                );
            }
